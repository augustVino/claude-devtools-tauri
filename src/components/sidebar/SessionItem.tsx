  ...
  return (
    <>
      <button
        ...
        className={`h-[48px] w-full select-none overflow-hidden border-b px-3 py-2 text-left transition-[background-color,opacity] duration-150 ${isActive ? '' : 'bg-transparent hover:opacity-80'} `}
        style={{
      ...
      >
        ...
        >
          ...
          <span className="tabular-nums">{formatShortTime(new Date(Math.max(session.updatedAt ?? session.createdAt, session.createdAt)))}</span>
