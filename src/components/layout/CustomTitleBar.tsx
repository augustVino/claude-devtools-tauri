 ...
 * on the right. Only shown on Windows or Linux (macOS uses native traffic lights).
 */
...
export const CustomTitleBar = (): React.JSX.Element | null => {
  ...
  const useNativeTitleBar = useStore(
    (s) => s.appConfig?.general?.useNativeTitleBar ?? false,
  );
  ...
  useEffect(() => {
    if (windowControls) void windowControls.isMaximized().then(setIsMaximized);
  }, [windowControls]);

  if (!showTitleBar || !windowControls) return null;
