...
mod tests {
    ...
    use crate::infrastructure::config_validator::{json_merge, validate_update_payload};

    fn temp_path() -> std::path::PathBuf {
        std::env::temp_dir().join(format!(
            "claude-devtools-test-{}.json",
            uuid::Uuid::new_v4()
        ))
    }
    ...
    async fn test_save_and_load_round_trip() {
        ...
        assert_eq!(
            m2.get_config().await.sessions.pinned_sessions["proj"][0].session_id,
            "sess"
        );
        cleanup(&p);
    }
    ...
    async fn test_add_remove_ignore_regex() {
        ...
        assert!(m
            .add_ignore_regex("test-pat".into())
            .await
            .unwrap()
            .notifications
            .ignored_regex
        ...
        assert!(!m
            .remove_ignore_regex("test-pat".into())
            .await
            .unwrap()
            .notifications
            .ignored_regex
    ...
    fn test_json_merge_deep() {
        let r = json_merge(
            &serde_json::json!({"a": 1, "b": {"c": 2, "d": 3}}),
            &serde_json::json!({"a": 10, "b": {"c": 20}}),
        );
        assert_eq!(r["a"], 10);
        assert_eq!(r["b"]["c"], 20);
        assert_eq!(r["b"]["d"], 3);
    ...
    fn test_validate_rejects_unknown_key_in_notifications() {
        let err =
            validate_update_payload("notifications", &serde_json::json!({"unknownKey": true}))
                .unwrap_err();
        assert!(err.contains("unknownKey is not supported"));
    }
    #[test]
    fn test_validate_rejects_unknown_key_in_general() {
        let err = validate_update_payload("general", &serde_json::json!({"unknownKey": "foo"}))
            .unwrap_err();
    ...
    fn test_validate_rejects_unknown_key_in_display() {
        assert!(
            validate_update_payload("display", &serde_json::json!({"unknownKey": 42}))
                .unwrap_err()
                .contains("not a valid setting")
        );
    }
    #[test]
    fn test_validate_rejects_unknown_key_in_http_server() {
        assert!(
            validate_update_payload("httpServer", &serde_json::json!({"unknownKey": true}))
                .unwrap_err()
                .contains("not a valid setting")
        );
    }
    ...
    fn test_validate_rejects_unknown_key_in_ssh() {
        assert!(
            validate_update_payload("ssh", &serde_json::json!({"unknownKey": true}))
                .unwrap_err()
                .contains("not a valid setting")
        );
    }
    ...
    fn test_validate_empty_object_passes() {
        for s in &["notifications", "general", "display", "httpServer", "ssh"] {
            assert!(validate_update_payload(s, &serde_json::json!({})).is_ok());
        }
    }
    #[test]
    fn test_validate_non_object_rejects() {
        assert!(
            validate_update_payload("general", &serde_json::json!("string"))
                .unwrap_err()
    ...
    fn test_validate_notifications_bool_fields() {
        assert!(
            validate_update_payload("notifications", &serde_json::json!({"enabled": true})).is_ok()
        );
        assert!(
            validate_update_payload("notifications", &serde_json::json!({"enabled": "true"}))
                .unwrap_err()
    ...
    fn test_validate_notifications_snooze_minutes_range() {
        assert!(
            validate_update_payload("notifications", &serde_json::json!({"snoozeMinutes": 1}))
                .is_ok()
        );
        assert!(validate_update_payload(
            "notifications",
            &serde_json::json!({"snoozeMinutes": 1441})
        )
        .unwrap_err()
    ...
    fn test_validate_notifications_ignored_regex_type() {
        assert!(validate_update_payload(
            "notifications",
            &serde_json::json!({"ignoredRegex": ["a"]})
        )
        .is_ok());
        assert!(validate_update_payload(
            "notifications",
            &serde_json::json!({"ignoredRegex": "x"})
        )
        .unwrap_err()
    ...
    fn test_validate_notifications_snoozed_until() {
        assert!(validate_update_payload(
            "notifications",
            &serde_json::json!({"snoozedUntil": null})
        )
        .is_ok());
        assert!(
            validate_update_payload("notifications", &serde_json::json!({"snoozedUntil": -1}))
                .unwrap_err()
                .contains("non-negative number or null")
        );
    }
    #[test]
    fn test_validate_general_theme_enum() {
        ...
        assert!(
            validate_update_payload("general", &serde_json::json!({"theme": "invalid"}))
                .unwrap_err()
    ...
    fn test_validate_general_claude_root_path() {
        assert!(
            validate_update_payload("general", &serde_json::json!({"claudeRootPath": "/abs"}))
                .is_ok()
        );
        assert!(
            validate_update_payload("general", &serde_json::json!({"claudeRootPath": "rel"}))
                .unwrap_err()
    ...
    fn test_validate_http_server_port_range() {
        ...
        assert!(
            validate_update_payload("httpServer", &serde_json::json!({"port": 0}))
                .unwrap_err()
    ...
    fn test_validate_sessions_always_passes() {
        assert!(
            validate_update_payload("sessions", &serde_json::json!({"anything": "goes"})).is_ok()
        );
    }
    #[test]
    fn test_normalize_root_path_strips_trailing_slash() {
        assert_eq!(
            path_utils::normalize_claude_root_path("/Users/foo/.claude/"),
            "/Users/foo/.claude"
        );
    }
    #[test]
    fn test_normalize_root_path_resolves_dot_segments() {
        assert_eq!(
            path_utils::normalize_claude_root_path("/Users/foo/../bar/.claude"),
            "/Users/bar/.claude"
        );
    }
    #[test]
    ...
    fn test_validate_notifications_ignored_repositories_type() {
        assert!(validate_update_payload(
            "notifications",
            &serde_json::json!({"ignoredRepositories": ["repo1"]})
        )
        .is_ok());
        let err = validate_update_payload(
            "notifications",
            &serde_json::json!({"ignoredRepositories": "not-array"}),
        )
        .unwrap_err();
    ...
    fn test_validate_notifications_triggers() {
        ...
        assert!(validate_update_payload(
            "notifications",
            &serde_json::json!({"triggers":[valid_trigger]})
        )
        .is_ok());
        // trigger without id
    ...
    fn test_validate_general_default_tab_enum() {
        assert!(
            validate_update_payload("general", &serde_json::json!({"defaultTab":"dashboard"}))
                .is_ok()
        );
        assert!(validate_update_payload(
            "general",
            &serde_json::json!({"defaultTab":"last-session"})
        )
        .is_ok());
        let err = validate_update_payload("general", &serde_json::json!({"defaultTab":"invalid"}))
            .unwrap_err();
    ...
    fn test_validate_general_bool_fields() {
        assert!(
            validate_update_payload("general", &serde_json::json!({"launchAtLogin":true})).is_ok()
        );
        assert!(
            validate_update_payload("general", &serde_json::json!({"showDockIcon":false})).is_ok()
        );
        assert!(validate_update_payload(
            "general",
            &serde_json::json!({"autoExpandAIGroups":true})
        )
        .is_ok());
        assert!(validate_update_payload(
            "general",
            &serde_json::json!({"useNativeTitleBar":false})
        )
        .is_ok());
        let err = validate_update_payload("general", &serde_json::json!({"launchAtLogin":"true"}))
            .unwrap_err();
    ...
    fn test_validate_ssh_profile_required_fields() {
        ...
        assert!(
            validate_update_payload("ssh", &serde_json::json!({"profiles":valid_profiles})).is_ok()
        );
        // missing id
    ...
    fn test_validate_ssh_last_connection_types() {
        assert!(
            validate_update_payload("ssh", &serde_json::json!({"lastConnection":null})).is_ok()
        );
        assert!(validate_update_payload("ssh", &serde_json::json!({"lastConnection":{}})).is_ok());
        let err = validate_update_payload("ssh", &serde_json::json!({"lastConnection":"string"}))
            .unwrap_err();
    ...
    fn test_validate_display_bool_fields() {
        assert!(
            validate_update_payload("display", &serde_json::json!({"showTimestamps":true})).is_ok()
        );
        let err = validate_update_payload("display", &serde_json::json!({"showTimestamps":"true"}))
            .unwrap_err();
        assert!(err.contains("must be a boolean"));
    }
    ...
    async fn test_update_trigger_infer_mode_when_mode_absent() {
        ...
        let result = m
            .update_trigger("infer-test", serde_json::json!({"name": "Updated"}))
            .await
            .unwrap();
        let updated = result
            .notifications
    ...
    async fn test_update_trigger_preserves_mode_when_explicitly_set() {
        ...
        m.initialize().await.unwrap();
        let trigger = crate::types::config::NotificationTrigger {
            id: "preserve-mode".to_string(),
            name: "Preserve Mode".to_string(),
            enabled: true,
            content_type: crate::types::config::TriggerContentType::ToolResult,
    ...
    async fn test_update_trigger_rejects_invalid_enum_value() {
        ...
        let result = m
            .update_trigger("enum-test", serde_json::json!({"mode": "invalid_mode"}))
            .await
            .unwrap();
        let updated = result
            .notifications
            .triggers
            .iter()
            .find(|t| t.id == "enum-test")
            .unwrap();
        assert_eq!(
            updated.mode,
            crate::types::config::TriggerMode::ErrorStatus,
            "invalid mode should be silently ignored"
        );
        cleanup(&p);
    }
    ...
    async fn test_hide_unhide_session_round_trip() {
        ...
        let c2 = m
            .unhide_session("proj".into(), "sess1".into())
            .await
            .unwrap();
        assert!(c2
            .sessions
