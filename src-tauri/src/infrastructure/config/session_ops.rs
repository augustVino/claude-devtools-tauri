...
impl super::ConfigManager {
    ...
    pub async fn pin_session(&self, project_id: String, session_id: String) -> crate::types::AppConfig {
    ...
    pub async fn unpin_session(&self, project_id: String, session_id: String) -> crate::types::AppConfig {
        self.with_config_mut(|config| {
    ...
    pub async fn hide_session(&self, project_id: String, session_id: String) -> crate::types::AppConfig {
        self.with_config_mut(|config| {
    ...
    pub async fn unhide_session(&self, project_id: String, session_id: String) -> crate::types::AppConfig {
        self.with_config_mut(|config| {
    ...
    pub async fn hide_sessions(&self, project_id: String, session_ids: Vec<String>) -> crate::types::AppConfig {
        if session_ids.is_empty() { return self.get_config().await; }
    ...
    pub async fn unhide_sessions(&self, project_id: String, session_ids: Vec<String>) -> crate::types::AppConfig {
        if session_ids.is_empty() { return self.get_config().await; }
    ...
    async fn with_config_mut<F>(&self, mutator: F) -> crate::types::AppConfig where F: FnOnce(&mut crate::types::AppConfig) -> bool {
        let mut config = self.config.write().await;
        let changed = mutator(&mut config);
        drop(config);
        if changed { let _ = self.persist_inner().await; }
        self.get_config().await
    }
}
