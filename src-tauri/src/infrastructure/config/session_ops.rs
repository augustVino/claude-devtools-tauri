...
impl super::ConfigManager {
    ...
    pub async fn pin_session(
        ...
        project_id: String,
        session_id: String,
    ) -> Result<crate::types::AppConfig, crate::error::AppError> {
            ...
            if !pins.iter().any(|p| p.session_id == session_id) {
                pins.insert(
                    ...
                    PinnedSession {
                        ...
                        pinned_at: super::defaults::now_millis(),
                    },
                );
                true
            } else {
        ...
        self.with_config_mut(|config| {
            ...
            if !hidden.iter().any(|h| h.session_id == session_id) {
                hidden.insert(
                    ...
                    HiddenSession {
                        session_id: session_id.clone(),
                        hidden_at: super::defaults::now_millis(),
                    },
                );
                true
            } else {
    ...
    pub(super) async fn with_config_mut<F>(
        &self,
        mutator: F,
    ) -> Result<crate::types::AppConfig, crate::error::AppError>
    where
